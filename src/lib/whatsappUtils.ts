...
export async function handleIncomingWhatsAppMessage(body: any) {
  ...
  try {
    ...
    if (text) {
      ...
      // Handle preference commands for earnings summaries
      const { parsePreferenceCommand, getUserPreferences, enableMonthlyReports, disableMonthlyReports, updateUserPreferences, formatPreferenceStatus } = await import('./userPreferencesService');
      const { previewMonthlySummary } = await import('./proactiveSummaryService');
      
      const preferenceCommand = parsePreferenceCommand(text);
      if (preferenceCommand.action) {
        console.log("Preference command detected:", preferenceCommand);
        
        try {
          switch (preferenceCommand.action) {
            case 'enable':
              const enableResult = await enableMonthlyReports(wallet.address);
              if (enableResult) {
                await sendWhatsAppMessage(from, {
                  text: "✅ Monthly earnings reports enabled! You'll receive automatic summaries on the 1st of each month.\n\n💡 Want to see what it looks like? Send \"preview monthly summary\""
                });
              } else {
                await sendWhatsAppMessage(from, {
                  text: "❌ Failed to enable monthly reports. Please try again."
                });
              }
              return;
              
            case 'disable':
              const disableResult = await disableMonthlyReports(wallet.address);
              if (disableResult) {
                await sendWhatsAppMessage(from, {
                  text: "✅ Monthly earnings reports disabled. You can re-enable them anytime by sending \"enable monthly reports\""
                });
              } else {
                await sendWhatsAppMessage(from, {
                  text: "❌ Failed to disable monthly reports. Please try again."
                });
              }
              return;
              
            case 'status':
              const preferences = await getUserPreferences(wallet.address);
              const statusMessage = formatPreferenceStatus(preferences);
              await sendWhatsAppMessage(from, { text: statusMessage });
              return;
              
            case 'currency':
              if (preferenceCommand.value) {
                const updateResult = await updateUserPreferences(wallet.address, {
                  preferredCurrency: preferenceCommand.value as string
                });
                if (updateResult) {
                  await sendWhatsAppMessage(from, {
                    text: `✅ Currency preference updated to ${preferenceCommand.value}. Your earnings summaries will now show values in ${preferenceCommand.value}.`
                  });
                } else {
                  await sendWhatsAppMessage(from, {
            ...
            case 'categories':
              if (preferenceCommand.value && Array.isArray(preferenceCommand.value)) {
                const updateResult = await updateUserPreferences(wallet.address, {
                  preferredCategories: preferenceCommand.value
                });
                if (updateResult) {
                  await sendWhatsAppMessage(from, {
                    text: `✅ Category preferences updated! You'll now see highlights for: ${preferenceCommand.value.join(', ')}`
                  });
                } else {
                  await sendWhatsAppMessage(from, {
                    text: "❌ Failed to update category preferences. Please try again."
                  });
                }
              }
              return;
          ...
          await sendWhatsAppMessage(from, {
            text: "❌ Error processing your preference command. Please try again."
          });
          return;
        }
      }

      // Handle monthly summary preview requests
      if (
        lowerText.includes("preview monthly") ||
        lowerText.includes("monthly preview") ||
        lowerText.includes("show monthly summary") ||
        lowerText.includes("test monthly")
      ) {
        console.log("Monthly summary preview requested");
        try {
          const preview = await previewMonthlySummary(wallet.address);
          await sendWhatsAppMessage(from, { text: preview });
          return;
        } catch (error) {
