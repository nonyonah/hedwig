...
export class USDCPaymentModule {
  ...
  private async generateBasePaymentInstructions(
    ...
    const message = (
      ...
      `*Token Contract:* \`${USDC_CONTRACTS.base}\`\n` +
      `*Recipient Address:*\n\`${recipientAddress}\`\n\n` +
      `*Instructions:*\n` +
      `1. Open your Web3 wallet (MetaMask, Coinbase Wallet, etc.)\n` +
      `2. Switch to Base network\n` +
      `3. Send ${amount.toFixed(2)} USDC to the address above\n` +
      `4. Click "Payment Sent" after transaction is confirmed\n\n` +
      `⚠️ *Important:* Only send USDC on Base network. Other tokens or networks will result in loss of funds.`
    );
