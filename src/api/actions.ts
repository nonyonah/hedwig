...
import { ProposalAgent, ProposalCollectionState } from '../lib/proposalAgent';
...
async function updateSession(userId: string, data: any) {
  await supabase
    .from('sessions')
    .upsert([{
      user_id: userId,
...
export async function handleAction(
  ...
  case "view_proposal":
    console.log(`[handleAction] Processing 'view_proposal' intent with params:`, params);
    return await handleViewProposal(params, userId);
...
async function handleCreatePaymentLink(params: any, userId: string) {
  try {
    ...
    const sendEmail = params.send_email; // New parameter to control email sending
...
async function handleCreateProposal(params: any, userId: string) {
  try {
    console.log(`[handleCreateProposal] Starting proposal creation with params:`, params);

    // Check if user has an existing proposal collection session
    const { data: session } = await supabase
      .from('sessions')
      .select('context')
      .eq('user_id', userId)
    ...
    let proposalState: ProposalCollectionState | null = null;

    // Look for existing proposal collection state in session context
    if (session?.context && Array.isArray(session.context)) {
      const proposalContext = session.context.find((item: any) => 
        item.role === 'system' && 
        item.content && 
        typeof item.content === 'string' && 
        item.content.includes('proposal_collection')
      );

      if (proposalContext) {
        try {
          const parsed = JSON.parse(proposalContext.content);
          if (parsed.proposal_collection) {
            proposalState = parsed.proposal_collection;
            console.log(`[handleCreateProposal] Found existing proposal state:`, proposalState);
          }
        } catch (e) {
          console.error(`[handleCreateProposal] Error parsing proposal context:`, e);
        }
      }
    }

    // Handle special commands
    const userText = params.text?.toLowerCase().trim();
    if (userText === 'restart' || userText === 'start over') {
      proposalState = ProposalAgent.initializeCollection();
      await updateSession(userId, { proposal_collection: proposalState });
      return { text: ProposalAgent.getCurrentQuestion(proposalState) };
    }

    if (userText === 'back' || userText === 'previous') {
      if (proposalState && proposalState.step > 0) {
        proposalState.step--;
        await updateSession(userId, { proposal_collection: proposalState });
        return { text: `Going back...\n\n${ProposalAgent.getCurrentQuestion(proposalState)}` };
      } else {
        return { text: "You're already at the beginning. Let's start collecting your proposal details." };
      }
    }

    if (userText === 'summary' && proposalState) {
      const summary = ProposalAgent.getCollectionSummary(proposalState);
      return { text: summary };
    }

    // If no existing state, initialize new proposal collection
    if (!proposalState) {
      proposalState = ProposalAgent.initializeCollection();
      await updateSession(userId, { proposal_collection: proposalState });
      return { text: ProposalAgent.getCurrentQuestion(proposalState) };
    ...
    // Process user input if we have text
    if (params.text) {
      const newState = ProposalAgent.processInput(proposalState, params.text);
      ...
      if (proposalState.isComplete) {
        // Validate all data is present
        const validation = ProposalAgent.validateCompleteData(proposalState.data);
        if (!validation.isValid) {
          return { text: `Missing information: ${validation.errors.join(', ')}\n\nPlease provide the missing details.` };
        }

        // Create the proposal
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/create-proposal`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              userId,
              proposalData: proposalState.data
            })
          });
          ...
          // Clear the proposal collection session
          await updateSession(userId, {});

          // Generate success message
          const summary = ProposalAgent.getCollectionSummary(proposalState);
          let successMessage = `✅ **Proposal Created Successfully!**\n\n${summary}\n\n📄 **Proposal ID:** ${proposalResult.proposal.id}`;
          
          if (proposalResult.paymentLink) {
            successMessage += `\n💳 **Payment Link:** ${proposalResult.paymentLink}`;
          }

          successMessage += `\n\n🚀 **Next Steps:**\n- Type "send proposal ${proposalResult.proposal.id}" to send it to your client\n- Type "view proposal ${proposalResult.proposal.id}" to see the full proposal`;

          return { text: successMessage };

        } catch (error) {
          console.error(`[handleCreateProposal] Error creating proposal:`, error);
          const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
          return { text: `Error creating proposal: ${errorMessage}\n\nPlease try again or type 'restart' to start over.` };
        }
      } else {
    ...
    // If no text provided, just return current question
    return { text: ProposalAgent.getCurrentQuestion(proposalState) };

  } catch (error) {
    console.error("[handleCreateProposal] Error:", error);
    return { text: "Error processing proposal request. Please try again or type 'restart' to start over." };
  }
}
...
async function handleSendProposal(params: any, userId: string) {
  try {
    ...
    // Extract proposal ID from params or text
    let proposalId = params.proposalId || params.proposal_id;
    
    if (!proposalId && params.text) {
      // Try to extract proposal ID from text like "send proposal 123"
      const match = params.text.match(/send\s+proposal\s+([a-zA-Z0-9-]+)/i);
      if (match) {
        proposalId = match[1];
      }
    }
    ...
    if (!proposalId) {
      // Get user's proposals to let them choose
      const { data: proposals, error } = await supabase
        .from('proposals')
        .select('id, client_name, project_title, status, created_at')
        .eq('user_id', userId)
        .order('created_at', { ascending: false })
        .limit(10);

      if (error) {
        console.error(`[handleSendProposal] Error fetching proposals:`, error);
        return { text: "Error fetching your proposals. Please try again." };
      }

      if (!proposals || proposals.length === 0) {
        return { text: "You don't have any proposals yet. Create one first by typing 'create proposal'." };
      }

      let proposalsList = "📋 **Your Recent Proposals:**\n\n";
      proposals.forEach((proposal, index) => {
        proposalsList += `${index + 1}. **${proposal.project_title}** for ${proposal.client_name}\n`;
        proposalsList += `   ID: ${proposal.id} | Status: ${proposal.status}\n`;
        proposalsList += `   Created: ${new Date(proposal.created_at).toLocaleDateString()}\n\n`;
      });

      proposalsList += "💬 **To send a proposal, type:** `send proposal [ID]`\n";
      proposalsList += "📖 **Example:** `send proposal ${proposals[0].id}`";

      return { text: proposalsList };
    }

    // Send the proposal
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/send-proposal`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          proposalId,
      ...
      let successMessage = `✅ **Proposal Sent Successfully!**\n\n`;
      successMessage += `📄 **Proposal:** ${result.proposal.project_title}\n`;
      successMessage += `👤 **Client:** ${result.proposal.client_name}\n`;
      successMessage += `📧 **Email:** ${result.proposal.client_email || 'Not provided'}\n`;
      successMessage += `🔗 **Proposal URL:** ${result.proposalUrl}\n\n`;
      
      if (result.whatsappSent) {
        successMessage += `📱 **WhatsApp confirmation sent to you**\n`;
      }
      
      if (result.emailSent) {
        successMessage += `📧 **Email sent to client**\n`;
      } else if (result.proposal.client_email) {
        successMessage += `⚠️ **Email sending failed** - You can share the proposal URL directly\n`;
      }

      return { text: successMessage };

    } catch (error) {
      console.error(`[handleSendProposal] Error sending proposal:`, error);
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return { text: `Error sending proposal: ${errorMessage}\n\nPlease try again or check the proposal ID.` };
    }

  } catch (error) {
    console.error("[handleSendProposal] Error:", error);
    return { text: "Error processing send proposal request. Please try again." };
  }
}

// Handler for viewing proposals
async function handleViewProposal(params: any, userId: string) {
  try {
    console.log(`[handleViewProposal] Viewing proposal with params:`, params);

    // Extract proposal ID from params or text
    let proposalId = params.proposalId || params.proposal_id;
    
    if (!proposalId && params.text) {
      // Try to extract proposal ID from text like "view proposal 123"
      const match = params.text.match(/(?:view|show)\s+proposal\s+([a-zA-Z0-9-]+)/i);
      if (match) {
        proposalId = match[1];
      }
    }
    ...
    if (!proposalId) {
      ...
      const { data: proposals, error } = await supabase
        .from('proposals')
        .select('id, client_name, project_title, status, created_at, payment_amount, service_fee')
        .eq('user_id', userId)
        .order('created_at', { ascending: false })
        .limit(10);

      if (error) {
        console.error(`[handleViewProposal] Error fetching proposals:`, error);
        return { text: "Error fetching your proposals. Please try again." };
      }
      ...
      let proposalsList = "📋 **Your Proposals:**\n\n";
      proposals.forEach((proposal, index) => {
        const totalAmount = proposal.payment_amount + proposal.service_fee;
        proposalsList += `${index + 1}. **${proposal.project_title}**\n`;
        proposalsList += `   Client: ${proposal.client_name}\n`;
        proposalsList += `   Amount: $${totalAmount.toLocaleString()}\n`;
        proposalsList += `   Status: ${proposal.status}\n`;
        proposalsList += `   ID: ${proposal.id}\n`;
        proposalsList += `   Created: ${new Date(proposal.created_at).toLocaleDateString()}\n\n`;
      });

      proposalsList += "💬 **To view a proposal, type:** `view proposal [ID]`\n";
      proposalsList += "📖 **Example:** `view proposal ${proposals[0].id}`";
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/get-proposal?proposalId=${proposalId}&userId=${userId}&includeSummary=true`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (!result.proposal) {
        return { text: "Proposal not found. Please check the ID and try again." };
      }

      const proposal = result.proposal;
      const summary = result.summary;
      
      let proposalDetails = `📄 **Proposal Details**\n\n`;
      proposalDetails += `**Project:** ${proposal.project_title}\n`;
      proposalDetails += `**Client:** ${proposal.client_name}\n`;
      proposalDetails += `**Status:** ${proposal.status}\n`;
      proposalDetails += `**Created:** ${new Date(proposal.created_at).toLocaleDateString()}\n\n`;
      
      proposalDetails += `**💰 Financial Details:**\n`;
      proposalDetails += `• Project Amount: $${proposal.payment_amount.toLocaleString()}\n`;
      proposalDetails += `• Service Fee: $${proposal.service_fee.toLocaleString()}\n`;
      proposalDetails += `• Total Amount: $${summary.totalAmount.toLocaleString()}\n`;
      proposalDetails += `• Payment Method: ${proposal.payment_method}\n\n`;
      
      proposalDetails += `**📅 Timeline:**\n`;
      proposalDetails += `• Start: ${new Date(proposal.timeline_start).toLocaleDateString()}\n`;
      proposalDetails += `• End: ${new Date(proposal.timeline_end).toLocaleDateString()}\n`;
      proposalDetails += `• Duration: ${summary.estimatedDuration}\n\n`;
      
      proposalDetails += `**📋 Description:**\n${proposal.description}\n\n`;
      proposalDetails += `**🎯 Deliverables:**\n${proposal.deliverables}\n\n`;
      
      if (proposal.client_email) {
        proposalDetails += `**📧 Client Email:** ${proposal.client_email}\n`;
      }
      
      if (proposal.payment_link_id) {
        proposalDetails += `**💳 Payment Link:** Available\n`;
      }
      
      proposalDetails += `\n**🚀 Actions:**\n`;
      proposalDetails += `• Send to client: \`send proposal ${proposal.id}\`\n`;
      proposalDetails += `• View full proposal: Visit the proposal URL when sent\n`;

      return { text: proposalDetails };

    } catch (error) {
      console.error(`[handleViewProposal] Error fetching proposal:`, error);
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return { text: `Error fetching proposal: ${errorMessage}\n\nPlease check the proposal ID and try again.` };
    }

  } catch (error) {
    console.error("[handleViewProposal] Error:", error);
    return { text: "Error processing view proposal request. Please try again." };
  }
}
