...
if (!ALCHEMY_AUTH_TOKEN) {
  console.error('ALCHEMY_AUTH_TOKEN environment variable is required');
  process.exit(1);
}
...
async function createAlchemyWebhook(network = 'BASE_MAINNET') {
  try {
    const networkName = network === 'BASE_SEPOLIA' ? 'Base Sepolia' : 'Base Mainnet';
    console.log('Creating Alchemy webhook...');
    console.log('Webhook URL:', WEBHOOK_URL);
    console.log(`Tracking USDC transfers on ${networkName}`);
...
switch (command) {
  case 'create':
    ...
    break;
  case 'create-sepolia':
    createAlchemyWebhook('BASE_SEPOLIA');
    break;
  case 'list':
    listWebhooks();
    break;
  ...
  default:
    ...
    console.log('');
    console.log('Usage:');
    console.log('  node setup-alchemy-webhook.js create           - Create new webhook (Base Mainnet)');
    console.log('  node setup-alchemy-webhook.js create-sepolia   - Create new webhook (Base Sepolia)');
